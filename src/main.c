...
loadConfig(const char *path)
{
	...
	for (i = 0; i < N_DEVICES; i++) {
		if (sceClibStrcmp(devices[i], getRoot((char *)sd0_path)) == 0)
			found_sd0 = 1;
		if (sceClibStrcmp(devices[i], getRoot((char *)ux0_path)) == 0)
			found_ux0 = 1;
	}
	if (found_sd0 < 1)
		sceClibSnprintf(sd0_path, sizeof(sd0_path), DEFAULT_SD0_PATH);
	if (found_ux0 < 1)
		sceClibSnprintf(ux0_path, sizeof(ux0_path), DEFAULT_UX0_PATH);
	return 0;
}
...
module_start(SceSize args, void *argp)
{
	...
	if (taiGetModuleInfo("SceVideoPlayer", &tai_info) >= 0) {
		...
		// check plugin state
		if (sceClibStrncmp(enabled, "false", 5) == 0)
...
module_stop(SceSize args, void *argp)
{
	...
	for (i = 0; i < N_HOOKS; i++)
		if (hook_uid[i] >= 0)
			taiHookRelease(hook_uid[i], hook_ref[i]);
	i = 0;
	for (i = n_uids - 1; i >= 0; i++)
		if (inject_uid[i] >= 0)
			taiInjectRelease(inject_uid[i]);
